...
class StudentManagementSystem:
    ...
    def create_db(self):
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS student (
                    ...
                    name TEXT)''')  # Removed photo column from the table
        conn.commit()
        conn.close()
            ...
            try:
                ...
                if row:
                    ...
                    cursor.execute("INSERT INTO student VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", (
                        ...
                        self.var_name.get()
                    ))  # Removed photo from the insert statement
                    conn.commit()
                    self.fetch_data()
                    conn.close()
    ...
    def fetch_data(self):
        conn = sqlite3.connect("student_small.db")
        cursor = conn.cursor()
        cursor.execute("SELECT id, dep, course, division, phone, email FROM student")  # Removed photo from the select statement
    ...
    def update_data(self):
        if self.var_std_id.get() == "":
            ...
            try:
                conn = sqlite3.connect("student_small.db")
                cursor = conn.cursor()
                cursor.execute("UPDATE student SET dep=?, course=?, year=?, semester=?, division=?, gender=?, phone=?, dob=?, email=?, address=?, name=? WHERE id=?", (
                    self.var_dep.get(), 
                    self.var_course.get(), 
                    self.var_year.get(), 
                    self.var_semester.get(), 
                    ...
                    self.var_name.get(), 
                    self.var_std_id.get()
                ))  # Removed photo from the update statement
                conn.commit()
                self.fetch_data()
                conn.close()
    ...
    def take_photo(self):
        if self.var_std_id.get() == "":
            ...
            try:
                ...
                path = r"C:\Users\KIIT\OneDrive\Documents\AD_Lab-current\open_ended_project\picture_click"
