...
class Attendance:
    def __init__(self, root):
        ...
        # Roll
        tk.Label(left_inside_frame, text="Roll No:", font=self.font_small, 
                bg="white").grid(row=0, column=2, padx=5, pady=5, sticky="w")
        ttk.Entry(left_inside_frame, textvariable=self.var_roll, 
        ...
        combo_search["values"] = ("ID", "Roll", "Name", "Department")
        ...
        self.attendance_table.column("roll", width=80, anchor=tk.CENTER)
    ...
    def add_data(self):
        if self.var_attendance_id.get() == "" or self.var_roll.get() == "":
            messagebox.showerror("Error", "ID and Roll fields are required", parent=self.root)
        else:
            try:
                conn = sqlite3.connect("attendance.db")
                cursor = conn.cursor()
    ...
    def update_data(self):
        if self.var_attendance_id.get() == "":
            ...
            try:
                conn = sqlite3.connect("attendance.db")
                cursor = conn.cursor()
                cursor.execute("UPDATE attendance SET roll=?, name=?, department=?, time=?, date=?, attendance=? WHERE id=?", (
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_department.get(),
                    self.var_time.get(),
        ...
        if self.var_search_by.get() == "" or self.var_search_txt.get() == "":
            ...
            try:
                ...
                if self.var_search_by.get() == "ID":
                    ...
                    cursor.execute("SELECT * FROM attendance WHERE roll LIKE ?", 
                                  ('%' + self.var_search_txt.get() + '%',))
